...
It prepares each scenario through `SimulationRuntimeFactory`, builds an
explicit prepared variant, and executes a bounded `RuntimeSession`; it is not
the API execution path and does not by itself prove API wiring or stochastic
fidelity.
...
Write current results outside the repository while iterating. Replace
`<temporary-directory>` with an OS-native temporary directory rather than a
tracked repository path:

```powershell
uv run python scripts/evaluate_scenarios.py --scenario <scenario-id> --output <temporary-directory>/scenario-evaluation-current.json --no-details --seed <seed>
```
...
```powershell
uv run python scripts/evaluate_scenarios.py --output <temporary-directory>/scenario-evaluation-current.json --no-details --seed <seed>
```
...
The harness supplies a recorder factory to `PreparedScenario.build`, executes
the returned session, and reads its recorder for diagnostics. Runtime
construction, victory evaluation, reinforcement scheduling, and finalization
remain owned by the factory/session path. Supplement the harness with focused
checkpoint, API, and end-to-end tests whenever the completion matrix requires
those boundaries.
