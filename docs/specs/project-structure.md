...
│   ├── ammunition/                   # Ammunition type definitions (AP, HE, HEAT, smoke, illum, guided)
...
└── stochastic_warfare/               # ===== MAIN PACKAGE =====
    ...
    │   ├── indirect_fire.py          # Artillery: fire missions, adjustment, counterbattery, area effects
...
### combat/
**Owns**: How things fight. All engagement types — direct fire, indirect fire, air-to-air, air-to-ground, ground-to-air. Hit probability, ballistic physics, damage, suppression, ammunition management, fratricide.
...
- **Indirect fire**: artillery fire missions (adjust fire, fire for effect, time-on-target), counterbattery radar and response, area effects, ammunition selection (HE, smoke, illumination, precision-guided)
...
- **Air defense**: SAM/AAA engagement envelopes (3D), shoot-look-shoot doctrine, EMCON states (radar on/off tradeoff between detection and self-protection)
- **Ammunition**: distinct types (AP, HE, HEAT, DPICM, smoke, illumination, guided) with different effects. Units select appropriate type. Ammo is consumed by type, not as a generic pool.
...
- **Coordination (c2/coordination.py)**: fire support coordination (FSCL, CFL, no-fire areas, restricted fire areas), airspace coordination (air corridors, transit routes, restricted operating zones), boundary management (lateral, rear), sea-land-air integration (composite warfare commander, amphibious force coordination). Failure of coordination → fratricide or missed opportunities.
