...
### Phase 18: CBRN — Chemical, Biological, Radiological, Nuclear (155 tests)
10 new source files + 6 modified + 15 YAML data files + 2 scenarios:
- **18a Agent Definitions, Events & Dispersal** (28 tests): `cbrn/__init__.py` (package), `cbrn/events.py` (10 event types), `cbrn/agents.py` (AgentCategory enum, AgentDefinition pydantic model, AgentRegistry), `cbrn/dispersal.py` (Pasquill-Gifford stability classification, Turner 1970 σy/σz coefficients, Gaussian puff concentration with ground reflection, wind advection, terrain channeling), `core/types.py` (+`ModuleId.CBRN`)
- **18b Contamination & Protection** (32 tests): `cbrn/contamination.py` (grid-based per-agent airborne/ground contamination, lazy grid allocation, exponential decay, temperature evaporation, rain washout, soil absorption, detection/cleared events), `cbrn/protection.py` (MOPPLevel enum, speed/detection/fatigue tables, protection factor with degradation, heat stress coupling)
- **18c Casualties & Decontamination** (25 tests): `cbrn/casualties.py` (DosageRecord, probit dose-response model Y=a+b·ln(D), Ct accumulation with protection factor, radiation dose accumulation, triage priority mapping), `cbrn/decontamination.py` (DeconType hasty/deliberate/thorough, duration+effectiveness tables, difficulty scaling, supply requirements)
- **18d Nuclear Effects** (30 tests): `cbrn/nuclear.py` (Hopkinson-Cranz blast overpressure, thermal fluence inverse-square, initial radiation with exponential attenuation, EMP radius scaling, blast/thermal casualty computation, EMP electronics disable, fallout plume generation, crater terrain modification, full detonation orchestrator)
- **18e Integration** (18 tests): `cbrn/engine.py` (CBRNEngine orchestrator, CBRNConfig, release_agent, detonate_nuclear, tick update cycle, MOPP level management, get_mopp_effects), `simulation/scenario.py` (+`cbrn_engine`), `simulation/engine.py` (+CBRN tick), `movement/engine.py` (+`mopp_speed_factor`), `morale/state.py` (+`cbrn_stress` parameter and cache key)
- **18f YAML Data & Validation** (22 tests): 7 agent YAMLs (VX, sarin, mustard, chlorine, hydrogen cyanide, anthrax, Cs-137), 3 nuclear weapon YAMLs (10kT, 100kT, 1MT), 3 delivery system YAMLs (artillery shell, aerial bomb, SCUD warhead), 2 validation scenarios (chemical defense, nuclear tactical)

Key features: Pasquill-Gifford atmospheric dispersal with Turner 1970 coefficients, grid-based contamination overlay (lazy allocation), MOPP 0-4 levels with speed/detection/fatigue degradation, probit dose-response casualty model calibrated to LCt50, Ct accumulation with MOPP protection factor, hasty/deliberate/thorough decontamination, Hopkinson-Cranz nuclear blast scaling, thermal fluence with 0.35 partition, prompt radiation with exponential attenuation, EMP electronics disable, wind-driven fallout plumes, crater terrain modification. All effects backward-compatible via `enable_cbrn` flag and default parameter values. Deterministic replay from seed.

No new dependencies.

### Phase 19: Doctrinal AI Schools (189 tests)
10 new source files + 6 modified + 9 YAML data files:
- **19a School Framework & Engine Hooks** (35 tests): `c2/ai/schools/__init__.py` (SchoolRegistry with unit assignments, SchoolLoader YAML loader), `c2/ai/schools/base.py` (SchoolDefinition pydantic model, DoctrinalSchool ABC with 8 hooks: assessment weight overrides, decision score adjustments, OODA multiplier, COA score weight overrides, risk tolerance, stratagem affinity, opponent prediction, opponent-adjusted scores), `c2/ai/assessment.py` (+`weight_overrides` param with re-normalization, +`predict_opponent_action_lanchester()` standalone function), `c2/ai/decisions.py` (+`school_adjustments` param threaded through all 5 echelon functions to `_select_best()`), `c2/ai/commander.py` (+`school_id` field on `CommanderPersonality`)
- **19b Western Schools** (65 tests): `c2/ai/schools/clausewitzian.py` (center-of-gravity, culmination awareness), `c2/ai/schools/maneuverist.py` (tempo-driven, bypass strongpoints), `c2/ai/schools/attrition.py` (exchange ratio optimization), `c2/ai/schools/airland_battle.py` (deep/close/rear, echelon-dependent), `c2/ai/schools/air_power.py` (Five Rings, air superiority prerequisite)
- **19c Eastern & Historical Schools** (31 tests): `c2/ai/schools/sun_tzu.py` (intel-first, deception, opponent modeling via Lanchester prediction + counter-posture adjustments), `c2/ai/schools/deep_battle.py` (echeloned assault, reserve management)
- **19d Maritime Schools** (15 tests): `c2/ai/schools/maritime.py` (MahanianSchool — fleet concentration, decisive battle, anti-dispersal; CorbettianSchool — fleet-in-being, sea denial, selective engagement)
- **19e Integration** (18 tests): `simulation/scenario.py` (+`school_registry` field on SimulationContext, included in get_state/set_state), `simulation/battle.py` (_process_ooda_completions wires school weight overrides to assessor, school adjustments + opponent modeling to decision engine, school OODA multiplier stacked with tactical_mult), `c2/planning/coa.py` (+`score_weight_overrides` param on `compare_coas()`)
- **19f YAML Data & Validation** (25 tests): 9 school YAML definitions (clausewitzian, maneuverist, attrition, airland_battle, air_power, sun_tzu, deep_battle, maritime_mahanian, maritime_corbettian)

Key features: Strategy-pattern schools producing modifier dicts injected via optional parameters (same DI pattern as mopp_speed_factor, fuel_available, jam_snr_penalty_db). YAML+Python hybrid — YAML stores numeric constants, Python subclasses add conditional logic (Clausewitzian culmination, Sun Tzu opponent modeling, AirLand echelon-dependent behavior). Assessment weight overrides multiplicatively applied then re-normalized. Opponent modeling via force-ratio Lanchester heuristic with counter-posture score adjustment. OODA multiplier stacks with tactical acceleration. All effects backward-compatible via `None` default parameter values. Deterministic replay from seed.
