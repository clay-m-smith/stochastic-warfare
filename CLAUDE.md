...
### Post-MVP Phases (11-22) — All Complete
...
### Phase 22 Detail (Most Recent)

6 new source files + 2 modified + ~53 YAML data files:
- **22a Era Config + Data** (102 tests): `core/era.py` (+NAPOLEONIC_ERA_CONFIG), `simulation/scenario.py` (+6 context fields). 10 units, 9 weapons, 9 ammo, 3 sensors, 10 signatures, 3 doctrines, 3 commanders, 2 comms.
- **22b Engine Extensions** (98 tests): `combat/volley_fire.py` (Binomial aggregate musket fire, smoke), `combat/melee.py` (pre-contact morale, force ratio combat, pursuit), `movement/cavalry.py` (charge state machine, fatigue), `movement/formation_napoleonic.py` (LINE/COLUMN/SQUARE/SKIRMISH, worst-of-both transitions), `c2/courier.py` (physical messenger, interception risk, courier pool), `logistics/foraging.py` (terrain productivity, depletion/recovery, ambush).
- **22c Validation Scenarios** (33 tests): Austerlitz, Waterloo.

Napoleonic pattern: CBRN fully disabled (unlike WW1). All 7 modern modules disabled. VISUAL-only sensors. Volley fire as aggregate Binomial model (like WW1 barrage). Formation as combat multiplier (rock-paper-scissors). Pre-contact morale decides most charges. Worst-of-both during formation transitions. c2_delay_multiplier=8.0. No new dependencies.
