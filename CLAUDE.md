...
### Phase 26 Detail (Most Recent)

23 source files modified (26a) + 9 source files modified (26b) + 5 source + 6 YAML (26c) + 12 test files updated + 3 new test files (82 tests):
- **26a PRNG Discipline** (25 tests): Removed `default_rng()` fallback from all 23 engine constructors. `rng` now required (keyword-only via `*,`). Zero matches for `default_rng` in source.
- **26b Configurable Constants** (34 tests): 9 source files — DispersalConfig (terrain channeling), CBRNConfig (weather fallbacks), GasWarfareConfig (wind angle), TrenchConfig (direction angles), ForagingConfig (ambush rate), JammingConfig (event radius), SpoofingEngine (unit_id param), EWDecoyConfig (effectiveness matrix as dict), SIGINTConfig (sigmoid params).
- **26c Engine Lifecycle** (23 tests): Puff cleanup (`max_puff_age_s`, `cleanup_aged_puffs()`), integration scan cap (`max_integration_scans`), armor_type field on GroundUnit/UnitDefinition + 6 YAML files.

Phase 26 pattern: Quality polish — no new engines or domains. All changes backward-compatible (new config fields have defaults matching original hardcoded values). Resolved 11 deficits (5.6, 5.7, 7.1, 7.2, 7.3, 8.1, 8.2, 8.3, 8.4, 10.4, 10.5).
