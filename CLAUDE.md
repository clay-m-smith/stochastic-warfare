...
### Phase 25 Detail (Most Recent)

5 modified source files + 4 new test files (152 tests):
- **25a ScenarioLoader Auto-Wiring** (61 tests): 5 new config blocks on `CampaignScenarioConfig`, 4 new `SimulationContext` fields, 7 `_create_*_engines()` sub-methods with lazy imports. `create_school()` factory in `c2/ai/schools/__init__.py`.
- **25d CommanderEngine Wiring** (22 tests): `_apply_commander_assignments()` side-level defaults + per-unit overrides. Commander OODA speed multiplier stacked with school multiplier in battle loop.
- **25b Battle Loop OODA Fix** (34 tests): `_cached_assessments` transient dict. OBSERVE uses real morale (`MoraleState` → 0.0–1.0) and real supply (`StockpileManager.get_supply_state()`). DECIDE retrieves cached assessment + commander personality. Opponent modeling with real force counts.
- **25c Tick Loop Integration** (34 tests): `strict_mode` parameter (re-raise vs log). 6 bare `except: pass` → `logger.error(exc_info=True)`. `_update_ew()` in tick loop. Insurgency with real military presence. MOPP speed factor in `_execute_movement()`.

Phase 25 pattern: Wiring only — no new engines or mechanics. Null-config = disabled (zero cost). Safe attribute access (`getattr(ctx, ..., None)`) for backward compat with SimpleNamespace test mocks. Resolved 9 deficits (1.1, 1.2, 1.3, 1.5, 1.6, 1.7, 2.8, 4.13, 5.1).
