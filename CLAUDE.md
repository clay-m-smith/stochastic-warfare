...
### Block 9 Phases (83–91) — Complete

| Phase | Focus | Tests | Key Deliverables |
|-------|-------|-------|------------------|
| 83 | Profiling Infrastructure | 13 | Benchmark suite (`BenchmarkResult`/`BenchmarkBaseline`/`run_benchmark()`), JSON baselines (73 Easting + Golan Heights), regression detection (20% margin), profiling tooling (hotspot reports, profile comparison, flame graph), CI benchmark workflow. 7 new files, 5 modified docs. |
| 84 | Spatial Culling & Scan Scheduling | 31 | STRtree detection culling in FOW (`enable_detection_culling=True`), sensor scan scheduling (`enable_scan_scheduling=False`, `scan_interval_ticks` on 33 sensors), engagement candidate culling via STRtree in `_execute_engagements()`. 4 modified source files, 33 sensor YAML, 3 test files. |
| 85 | LOD & Aggregation | 30 | UnitLodTier (ACTIVE/NEARBY/DISTANT), `_classify_lod_tiers()` with hysteresis, scheduler-based update frequency (1/5/20 ticks), LOD gates in detection/morale/supply/engagement, order preservation in aggregation snapshots. 3 modified source files, 3 test files. |
| 86 | Engagement & Calibration Optimization | 19 | `CalibrationSchema.to_flat_dict()` replaces pydantic `.get()` with O(1) dict lookup (98 call sites), `_ObserverModifiers` NamedTuple batches MOPP/altitude/readiness per-observer, `_resolve_cal_flat()` backward compat helper. 3 modified source files, 2 test files. |
| 87 | Expanded Numba JIT | 40 | 8 `@optional_jit` kernels: detection (5 SNR/Pd), combat (hit probability + penetration), morale (discrete + continuous transition matrices). Extract-and-delegate pattern, `math.erfc` replaces `scipy.special.erfc`. 4 modified source files, 3 test files. |
| 88 | SoA Data Layer | 43 | `UnitArrays` class — SoA for positions/health/fuel/morale/operational/max_range. `from_units()` builds arrays, `get_enemy_positions()` replaces `_build_enemy_data` position logic, `distance_matrix()` via cdist. FOW vectorized range check. Gated behind `enable_soa: bool = False`. 1 new + 3 modified source files, 3 test files. |
| 89 | Per-Side Parallelism | 21 | ThreadPoolExecutor per-side FOW detection dispatch, RNG forking via `integers()` draw for deterministic per-side streams, `rng` param injection through `FOW.update()` → `DetectionEngine.check_detection()`. Movement parallelism descoped (GIL). Gated behind `enable_parallel_detection: bool = False`. 4 modified source files, 2 test files. |
| 90 | Validation & Benchmarking | ~20 | Battalion (1,000 unit) and brigade (5,000 unit) benchmark scenarios, tightened perf targets (73E <15s, Golan <60s), `calibration_overrides` param on `run_benchmark()`, flag impact matrix (5 perf flags individually + combined). 2 new scenario YAML, 1 new + 2 modified test files, baselines updated. |
| 91 | Scenario Recalibration & Regression | ~20 | 4 perf flags enabled on 30 scenarios (26 modern + 4 WW2), `_DEFERRED_FLAGS` reduced 7→2, Block 9 regression test suite (`test_block9_regression.py`), Block 9 retrospective. 30 scenario YAML modified, 1 new + 1 modified test file. Block 9 COMPLETE. |

### Block 10 Phases (92–97) — In Progress

| Phase | Focus | Tests | Key Deliverables |
|-------|-------|-------|------------------|
| 92 | API Analytics & Frame Enrichment | 20 | 5 analytics endpoints (casualties, suppression, morale, engagements, summary), enriched MapUnitFrame (7 fields: morale/posture/health/fuel/ammo/suppression/engaged), 4 metadata endpoints (schools/commanders/weapons list+detail). 2 new + 4 modified API files. Zero engine changes. |
| 93 | Results Dashboard Depth | 14 | 4 new Plotly chart components (CasualtyBreakdown, EngagementSummary, Suppression, MoraleDistribution), analytics summary card on ResultsTab, TanStack Query hooks + API client + TypeScript types. 7 new + 2 modified frontend files. TS test fixes (6 files). |
| 94 | Tactical Map Enrichment | 42 | 5 overlay toggles (morale color/health bars/posture icons/suppression opacity/logistics bars), engagement flash, enhanced unit detail sidebar (7 enriched fields), 5 conditional legend sections. 6 modified + 4 new test files. Zero engine changes. |
