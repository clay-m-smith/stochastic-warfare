...
### Post-MVP Phases (11-23) — All Complete
...
### Phase 23 Detail (Most Recent)

5 new source files + 4 modified + ~49 YAML data files:
- **23a Era Config + Data** (167 tests): `core/era.py` (+ANCIENT_MEDIEVAL_ERA_CONFIG), `simulation/scenario.py` (+5 context fields), `validation/historical_data.py` + `scenario_runner.py` (+open_field terrain). 7 units, 13 weapons, 8 ammo, 3 sensors, 7 signatures, 3 doctrines, 3 commanders, 2 comms.
- **23b Engine Extensions** (112 tests): `combat/archery.py` (Binomial massed archery, 5 missile types, armor reduction, per-archer ammo tracking), `movement/formation_ancient.py` (7 formations: PHALANX/SHIELD_WALL/PIKE_BLOCK/WEDGE/SKIRMISH/TESTUDO/COLUMN), `combat/siege.py` (campaign-scale daily state machine: ENCIRCLEMENT→BOMBARDMENT→BREACH→ASSAULT→FALLEN/RELIEF), `movement/naval_oar.py` (fatigue-based rowing, ramming, boarding), `c2/visual_signals.py` (banner/horn/runner/fire beacon, synchronous presence-based C2), `combat/melee.py` (+PIKE_PUSH/SHIELD_WALL/MOUNTED_CHARGE types, reach advantage, flanking multiplier).
- **23c Validation Scenarios** (42 tests): Cannae (216 BC), Agincourt (1415), Hastings (1066).

Ancient/Medieval pattern: All 7 modern modules disabled. VISUAL-only sensors. c2_delay_multiplier=12.0. Archery as aggregate Binomial model (same pattern as volley fire). Formation dominates combat outcomes. Reach advantage on first round only. Flanking multiplier 2.5x. Siege as daily state machine. Visual C2 limits coordination to 500-1000m. Morale/rout cascade as primary battle-ending mechanism. No new dependencies.
