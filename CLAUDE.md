...
| 86 | Engagement & Calibration Optimization | 19 | `CalibrationSchema.to_flat_dict()` replaces pydantic `.get()` with O(1) dict lookup (88 call sites), `_ObserverModifiers` NamedTuple batches MOPP/altitude/readiness per-observer, `_resolve_cal_flat()` backward compat helper. 3 modified source files, 2 test files. |
