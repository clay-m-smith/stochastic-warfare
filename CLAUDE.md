...
| 36 | Scenario Tweaker & Polish | ~59 | Clone-and-tweak editor (useReducer, 11 actions), POST /validate + /from-config endpoints, ForceEditor + UnitPicker + ConfigToggles + CalibrationSliders + YamlPreview + TerrainPreview. Export: JSON/CSV/YAML download + PrintReportPage. Polish: keyboard shortcuts (Space/arrows/1-4), responsive sidebar, ErrorMessage variants, WS reconnect (backoff). ~20 new frontend + 3 modified API + 12 test files. |
