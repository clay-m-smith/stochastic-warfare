...
async def test_lifespan_uses_factory_settings(
    ...
    factory_data = tmp_path / "factory-data"
    shutil.copytree(Path(__file__).resolve().parents[2] / "data", factory_data)
    shutil.rmtree(factory_data / "scenarios")
    (factory_data / "scenarios").mkdir()
    settings = ApiSettings(
        db_path=str(tmp_path / "factory.db"),
        data_dir=str(factory_data),
