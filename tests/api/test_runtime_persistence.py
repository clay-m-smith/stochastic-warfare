...
_ASYNC_COMPLETION_TIMEOUT_S = 30.0
_STATUS_POLL_INTERVAL_S = 0.025
...
async def _wait_for_status(
    ...
    statuses: set[str],
    *,
    timeout_s: float = _ASYNC_COMPLETION_TIMEOUT_S,
) -> dict:
    ...
    while True:
        ...
        if remaining_s <= 0:
            ...
            pytest.fail(
                f"{path} did not reach {sorted(statuses)!r} within "
        ...
        try:
            ...
            elapsed_s = time.monotonic() - started_at
            pytest.fail(
                f"{path} status request on attempt {attempts} did not "
                f"complete within the {timeout_s:.1f}s deadline "
                f"(elapsed={elapsed_s:.3f}s, last_status={last_status!r}, "
                f"last_payload={last_payload!r})",
            )
        if response.status_code != 200:
            pytest.fail(
                f"{path} status poll returned HTTP {response.status_code} on attempt {attempts}: {response.text!r}",
            )
        ...
        if status in _TERMINAL_STATUSES:
            pytest.fail(
                f"{path} reached unexpected terminal status {status!r}; "
                f"expected one of {sorted(statuses)!r}: {payload!r}",
            )
        if status not in _PENDING_STATUSES:
            pytest.fail(
                f"{path} status poll returned unexpected status {status!r} on attempt {attempts}: {payload!r}",
            )
        ...
        await asyncio.sleep(min(_STATUS_POLL_INTERVAL_S, remaining_s))


async def _wait_for_publication_boundary(
    publication_boundaries: asyncio.Queue[asyncio.Event],
    *,
    deadline: float,
    label: str,
) -> asyncio.Event:
    started_at = time.monotonic()
    remaining_s = deadline - started_at
    if remaining_s <= 0:
        pytest.fail(
            f"{label} publication boundary was not reached before the shared "
            f"{_ASYNC_COMPLETION_TIMEOUT_S:.1f}s deadline "
            f"(total_elapsed={started_at - (deadline - _ASYNC_COMPLETION_TIMEOUT_S):.3f}s, "
            f"queue_size={publication_boundaries.qsize()})",
        )
    try:
        ...
        pytest.fail(
            f"{label} publication boundary was not reached before the shared "
            f"{_ASYNC_COMPLETION_TIMEOUT_S:.1f}s deadline "
            f"(total_elapsed={finished_at - (deadline - _ASYNC_COMPLETION_TIMEOUT_S):.3f}s, "
            f"phase_elapsed={finished_at - started_at:.3f}s, "
            f"phase_budget={remaining_s:.3f}s, queue_size={publication_boundaries.qsize()})",
        )


async def _wait_for_publication_task(
    task,
    *,
    deadline: float,
    label: str,
    ...
    if remaining_s <= 0:
        pytest.fail(
            f"{label} terminal publication did not finish before the shared "
            f"{_ASYNC_COMPLETION_TIMEOUT_S:.1f}s deadline "
            f"(total_elapsed={started_at - (deadline - _ASYNC_COMPLETION_TIMEOUT_S):.3f}s, "
    ...
    try:
        ...
        pytest.fail(
            ...
            f"(total_elapsed={finished_at - (deadline - _ASYNC_COMPLETION_TIMEOUT_S):.3f}s, "
            f"phase_elapsed={finished_at - started_at:.3f}s, "
            f"phase_budget={remaining_s:.3f}s, done={task.done()}, "
...
async def test_accepted_cancellation_prevents_completion_publication(
    ...
    publication_deadline = time.monotonic() + _ASYNC_COMPLETION_TIMEOUT_S
    ...
    run_publication = await _wait_for_publication_boundary(
    ...
    run_publication.set()
    await _wait_for_publication_task(
        task,
        deadline=publication_deadline,
        label=f"run {run_id!r}",
    )
    ...
    batch_publication = await _wait_for_publication_boundary(
        publication_boundaries,
        deadline=publication_deadline,
        label=f"batch {batch_id!r}",
    )
