...
def test_stored_side_projection_requires_privileged_associations() -> None:
    ...
    with pytest.raises(ValueError, match="lacks privileged track associations"):
        _frame_from_storage(
            stored,
            scope=TargetingExposureScope.SIDE_FOW,
            side="blue",
...
async def test_api_side_scope_never_derives_from_legacy_privileged_frame(
    ...
    body = privileged.json()
    assert body["scope"] == "PRIVILEGED_ENGINE"
    assert body["frames"][0]["scope"] == "PRIVILEGED_ENGINE"
