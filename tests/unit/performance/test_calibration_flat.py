...
class TestResolvedCalibration:
    """Immutable production calibration preserves the flat read contract."""

    def test_resolve_compiles_typed_immutable_projection(self) -> None:
        calibration = CalibrationSchema(
            defensive_sides=["blue"],
            target_value_weights={"armor": 2.0},
            side_overrides={"blue": SideCalibration(cohesion=0.91)},
        )

        resolved = calibration.resolve(["blue", "red"])

        assert type(resolved) is ResolvedCalibration
        assert resolved.sides == ("blue", "red")
        assert resolved.get("blue_cohesion") == pytest.approx(0.91)
        assert resolved["defensive_sides"] == ("blue",)
        assert dict(resolved["target_value_weights"]) == {"armor": 2.0}
        with pytest.raises(TypeError):
            resolved["enable_fog_of_war"] = True  # type: ignore[index]
        with pytest.raises(TypeError):
            resolved["target_value_weights"]["armor"] = 9.0

    def test_flat_dict_is_detached_from_resolved_owner(self) -> None:
        calibration = CalibrationSchema(defensive_sides=["blue"])
        resolved = calibration.resolve(["blue", "red"])

        compatibility = resolved.to_dict()
        compatibility["defensive_sides"].append("red")

        assert resolved["defensive_sides"] == ("blue",)
        assert calibration.to_flat_dict(["blue", "red"])[
            "defensive_sides"
        ] == ["blue"]
