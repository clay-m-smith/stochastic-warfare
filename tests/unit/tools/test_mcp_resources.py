...
from stochastic_warfare.tools import mcp_resources
...
class TestMCPResources:
    ...
    @pytest.mark.parametrize(
        "name",
        (
            "../eras/ww2/scenarios/waterloo",
            "%2e%2e%2feras%2fww2%2fscenarios%2fwaterloo",
            "test_campaign/config",
            r"test_campaign\config",
            "/tmp/test_campaign",
            ".",
            "..",
        ),
    )
    def test_scenario_resource_rejects_non_identifier_names(
        self,
        name: str,
    ) -> None:
        parsed = json.loads(get_scenario_config(name))

        assert parsed["error"] is True
        assert "Invalid scenario resource" in parsed["message"]

    @pytest.mark.parametrize(
        ("category", "unit_type"),
        (
            ("../validation", "historical_claims"),
            ("%2e%2e%2fvalidation", "historical_claims"),
            ("armor/modern", "m1a2"),
            (r"armor\modern", "m1a2"),
            ("armor", "m1a2/../../validation/historical_claims"),
            ("/tmp", "unit"),
        ),
    )
    def test_unit_resource_rejects_non_identifier_fields(
        self,
        category: str,
        unit_type: str,
    ) -> None:
        parsed = json.loads(get_unit_definition(category, unit_type))

        assert parsed["error"] is True
        assert "Invalid unit resource" in parsed["message"]

    @pytest.mark.parametrize(
        "run_id",
        ("../run", "%2e%2e%2frun", "group/run", r"group\run", "/tmp/run"),
    )
    def test_result_resource_rejects_non_identifier_run_ids(
        self,
        run_id: str,
    ) -> None:
        store = ResultStore()
        store.store(_make_stored(run_id))

        parsed = json.loads(get_cached_result(run_id, store))

        assert parsed["error"] is True
        assert "Invalid result resource" in parsed["message"]

    def test_scenario_resource_rejects_symlinked_directory(
        self,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        catalog_root = tmp_path / "catalog"
        scenario_root = catalog_root / "scenarios"
        outside = tmp_path / "outside-scenario"
        scenario_root.mkdir(parents=True)
        outside.mkdir()
        (outside / "scenario.yaml").write_text("name: escaped\n", encoding="utf-8")
        (scenario_root / "linked").symlink_to(outside, target_is_directory=True)
        paths = replace(
            mcp_resources._application_paths(),
            catalog_root=catalog_root,
            scenario_root=scenario_root,
        )
        monkeypatch.setattr(mcp_resources, "_application_paths", lambda: paths)

        parsed = json.loads(get_scenario_config("linked"))

        assert parsed["error"] is True
        assert "symlinks" in parsed["message"]

    def test_unit_resource_rejects_symlinked_file(
        self,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        catalog_root = tmp_path / "catalog"
        unit_root = catalog_root / "units" / "armor"
        unit_root.mkdir(parents=True)
        outside = tmp_path / "outside-unit.yaml"
        outside.write_text("unit_type: escaped\n", encoding="utf-8")
        (unit_root / "linked.yaml").symlink_to(outside)
        paths = replace(
            mcp_resources._application_paths(),
            catalog_root=catalog_root,
            scenario_root=catalog_root / "scenarios",
        ...
        parsed = json.loads(get_unit_definition("armor", "linked"))
