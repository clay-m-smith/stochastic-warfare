...
class TestOrderPropagationBehavior:
    ...
    def test_per_call_overrides_are_deterministic_and_do_not_mutate_config(self):
        config = PropagationConfig(
            delay_sigma=0.8,
            base_misinterpretation=0.10,
        )
        overrides = PropagationOverrides(
            delay_sigma=0.0,
            base_misinterpretation=0.0,
        )

        def run() -> tuple[object, dict]:
            engine = OrderPropagationEngine(
                _make_comms(),
                None,
                EventBus(),
                _make_rng(117),
                config=PropagationConfig(
                    delay_sigma=config.delay_sigma,
                    base_misinterpretation=config.base_misinterpretation,
                ),
            )
