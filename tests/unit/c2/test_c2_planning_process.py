...
class TestCheckpoint:
    ...
    def test_completed_result_round_trips_exactly(
        self,
        event_bus: EventBus,
        ...
        engine.initiate_planning(
            "unit_a",
            _make_order(order_id="plan_unit_a_2024-01-01"),
            10_000,
            ts=TS,
        )
        engine.complete_planning("unit_a", ts=TS)
        ...
        assert snapshot["checkpoint_schema"] == 1
        assert snapshot["states"]["unit_a"]["selected_result"] == {
            "kind": "planning_result",
            "value": "ATTACK",
        }
        restored = PlanningProcessEngine(event_bus, rng)
        restored.set_state(snapshot)

        assert restored.get_state() == snapshot
        assert restored.consume_result("unit_a") == "ATTACK"
        assert restored.consume_result("unit_a") is None

    def test_markerless_production_completion_migrates_default_result(
        self,
        event_bus: EventBus,
        rng: np.random.Generator,
    ) -> None:
        ...
        markerless = copy.deepcopy(current)
        markerless.pop("checkpoint_schema")
        markerless["states"]["unit_a"].pop("selected_result")

        restored = PlanningProcessEngine(event_bus, rng)
        restored.set_state(markerless)

        assert restored.get_state() == current
        assert restored.get_planning_result("unit_a") == "ATTACK"

    def test_markerless_custom_selection_rejects_atomically(
        self,
        event_bus: EventBus,
        rng: np.random.Generator,
    ) -> None:
        source = PlanningProcessEngine(event_bus, rng)
        source.initiate_planning("unit_a", _make_order(), 10_000, ts=TS)
        source.set_selected_coa("unit_a", "DEFEND")
        source.complete_planning("unit_a", ts=TS)
        markerless = source.get_state()
        markerless.pop("checkpoint_schema")
        markerless["states"]["unit_a"].pop("selected_result")
        target = PlanningProcessEngine(event_bus, rng)
        before = target.get_state()

        with pytest.raises(
            ValueError,
            match="cannot reconstruct its selected COA",
        ):
            target.set_state(markerless)
    ...
    @pytest.mark.parametrize(
        "mutation",
        [
            {"kind": "planning_result", "value": 1},
            {"kind": "unknown", "value": "ATTACK"},
            {"value": "ATTACK"},
            None,
        ],
    )
    def test_current_selected_result_tampering_rejects_atomically(
        self,
        event_bus: EventBus,
        rng: np.random.Generator,
        mutation: object,
        ...
        source.complete_planning("unit_a", ts=TS)
        tampered = source.get_state()
        tampered["states"]["unit_a"]["selected_result"] = mutation
        target = PlanningProcessEngine(event_bus, rng)
        before = target.get_state()

        with pytest.raises(ValueError, match="selected"):
            target.set_state(tampered)
