...
class _RecordingDecisionEngine:
    ...
    def decide(self, **kwargs: Any) -> None:
        self.unit_ids.append(kwargs["unit_id"])


class _MappedCommunicationsEngine:
    def __init__(self, effectiveness: dict[str, float]) -> None:
    ...
    def compute_c2_effectiveness(
        ...
        *,
        ...
        self.calls.append(
            (unit_id, tuple(sorted(positions)), min_effectiveness),
        )
        return self.effectiveness[unit_id]


class _MappedMOPPState:
    def __init__(self, levels: dict[str, int]) -> None:
        self._levels = levels

    def get_mopp_level(self, unit_id: str) -> int:
        return self._levels.get(unit_id, 0)


def _c2_friction_context(
    *,
    effectiveness: dict[str, float],
    enabled: bool = True,
    human_factors: bool = False,
    mopp_levels: dict[str, int] | None = None,
) -> tuple[SimulationContext, _RecordingDecisionEngine, _MappedCommunicationsEngine]:
    raw_config = _minimal_config().model_dump(mode="python")
    raw_config["calibration_overrides"].update(
        {
            "enable_c2_friction": enabled,
            "c2_min_effectiveness": 0.6,
            "enable_human_factors": human_factors,
            "mopp_comms_factor_4": 0.5,
        },
    )
    config = CampaignScenarioConfig.model_validate(raw_config)
    units = [_make_unit(unit_id) for unit_id in effectiveness]
    decision = _RecordingDecisionEngine()
    comms = _MappedCommunicationsEngine(effectiveness)
    ctx = _make_ctx(
        config=config,
        calibration=config.calibration_overrides,
        units_by_side={"blue": units, "red": []},
        decision_engine=decision,
        comms_engine=comms,
        cbrn_engine=(
            _MappedMOPPState(mopp_levels or {})
            if human_factors
            else None
        ),
    )
    ooda = OODALoopEngine(
        ...
        OODAConfig(timing_sigma=0.0),
    )
    for unit_id in effectiveness:
        ooda.register_commander(unit_id, EchelonLevel.BATTALION)
        ooda.start_phase(unit_id, OODAPhase.DECIDE, ts=TS)
    ctx.ooda_engine = ooda
    return ctx, decision, comms


def _c2_manager(*unit_ids: str) -> BattleManager:
    manager = _make_bm()
    for unit_id in unit_ids:
        manager._cached_assessments[unit_id] = _assessment(unit_id)
    return manager


class TestC2FrictionGate:
    """Production OODA completion routing applies C2 friction per unit."""

    def test_due_units_are_independently_gated_by_c2_effectiveness(self) -> None:
        ctx, decision, comms = _c2_friction_context(
            effectiveness={"low": 0.2, "high": 0.8},
        )
        manager = _c2_manager("low", "high")

        manager._process_ooda_completions(
            ctx,
            [("low", OODAPhase.DECIDE), ("high", OODAPhase.DECIDE)],
            TS,
        )

        assert decision.unit_ids == ["high"]
        assert [call[0] for call in comms.calls] == ["low", "high"]
        assert all(call[1] == ("high", "low") for call in comms.calls)
        assert all(call[2] == pytest.approx(0.6) for call in comms.calls)
        assert ctx.ooda_engine.get_phase("low") is OODAPhase.ACT
        assert ctx.ooda_engine.get_phase("high") is OODAPhase.ACT

    def test_disabled_friction_does_not_query_or_gate_low_comms(self) -> None:
        ctx, decision, comms = _c2_friction_context(
            effectiveness={"low": 0.0},
            enabled=False,
        )

        _c2_manager("low")._process_ooda_completions(
            ctx,
            [("low", OODAPhase.DECIDE)],
            TS,
        )

        assert decision.unit_ids == ["low"]
        assert comms.calls == []
        assert ctx.ooda_engine.get_phase("low") is OODAPhase.ACT

    def test_mopp4_compounds_with_comms_before_the_per_unit_gate(self) -> None:
        ctx, decision, comms = _c2_friction_context(
            effectiveness={"mopp0": 0.8, "mopp4": 0.8},
            human_factors=True,
            mopp_levels={"mopp0": 0, "mopp4": 4},
        )

        _c2_manager("mopp0", "mopp4")._process_ooda_completions(
            ctx,
            [("mopp0", OODAPhase.DECIDE), ("mopp4", OODAPhase.DECIDE)],
            TS,
        )

        assert decision.unit_ids == ["mopp0"]
        assert [call[0] for call in comms.calls] == ["mopp0", "mopp4"]
        assert ctx.ooda_engine.get_phase("mopp0") is OODAPhase.ACT
        assert ctx.ooda_engine.get_phase("mopp4") is OODAPhase.ACT
