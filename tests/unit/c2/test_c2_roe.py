...
class TestWeaponsHold:
    ...
    def test_typed_default_configuration_updates_engine_state(self) -> None:
        bus = EventBus()
        roe = RoeEngine(bus, default_level=RoeLevel.WEAPONS_HOLD)

        roe.configure_default_level(RoeLevel.WEAPONS_FREE)
        authorized, reason = roe.check_engagement_authorized(
            "plt1",
            "enemy1",
            TargetCategory.MILITARY_COMBATANT,
            id_confidence=1.0,
            timestamp=_TS,
        )
        ...
        assert roe.get_state()["default_level"] == int(
