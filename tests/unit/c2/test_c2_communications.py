...
class TestEmcon:
    ...
    def test_unregistered_endpoint_has_no_channel(self) -> None:
        engine, _ = _make_engine(equipment=[_make_vhf()])
        engine.register_unit("u1", ["test_vhf"])

        assert engine.get_best_channel("u1", "missing", _POS_A, _POS_B) is None
        assert engine.get_best_channel("missing", "u1", _POS_A, _POS_B) is None
        assert engine.can_communicate("u1", "missing", _POS_A, _POS_B) is False
