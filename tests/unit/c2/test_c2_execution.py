...
class TestStateProtocol:
    ...
    def test_record_transaction_installs_and_restores_exact_topology(self) -> None:
        eng, _ = _make_exec_engine()
        eng.issue_order(_make_order("original"), _POS_A, _POS_B, _TS)
        original = eng.get_record("original")
        before = eng.get_state()
        snapshot = eng.capture_record_snapshot()

        restored_record = OrderExecutionRecord(
            order_id="restored",
            recipient_id="co1",
            status=OrderStatus.RECEIVED,
        )
        eng.install_records((restored_record,))
        assert eng.get_record("restored") is restored_record

        eng.restore_record_snapshot(snapshot)
        assert eng.get_state() == before
        assert eng.get_record("original") is original
        with pytest.raises(KeyError):
            eng.get_record("restored")

        other, _ = _make_exec_engine()
        with pytest.raises(ValueError, match="belongs to another engine"):
            other.restore_record_snapshot(snapshot)

    def test_record_installation_prevalidates_before_mutation(self) -> None:
        eng, _ = _make_exec_engine()
        valid = OrderExecutionRecord(order_id="valid", recipient_id="bn1")
        invalid = OrderExecutionRecord(order_id="", recipient_id="bn1")

        with pytest.raises(ValueError, match="require an order_id"):
            eng.install_records((valid, invalid))
        with pytest.raises(KeyError):
            eng.get_record("valid")
