...
class TestWW2EngineWiring:
    ...
    @pytest.mark.test_evidence("structural_only")
    def test_convoy_exception_uses_explicit_degraded_handler(
        self,
    ):
        """An explicit degraded callback records and contains the failure."""
        from stochastic_warfare.simulation.campaign import CampaignManager
        ...
        mgr = CampaignManager(ctx.event_bus, np.random.default_rng(42))
        failures = []
        mgr.update_strategic(
            ctx,
            3600.0,
            failure_handler=lambda subsystem, operation, exc: (
                failures.append((subsystem, operation, exc)) or True
            ),
        )
        convoy_eng.update_convoy.assert_called_once_with("c1", 3600.0)
        assert failures == [("logistics.convoy", "update", convoy_eng.update_convoy.side_effect)]
...
class TestWW1EngineWiring:
    ...
    def test_barrage_exception_emits_structural_diagnostic(
        ...
        assert any("combat.barrage.update" in record.getMessage() for record in caplog.records)
...
class TestSpaceVerification:
    ...
    def test_get_gps_cep_exception(self):
        """get_gps_cep() does not hide an enabled GPS owner fault."""
        from stochastic_warfare.space.constellations import SpaceEngine
