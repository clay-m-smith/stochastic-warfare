...
def _ledger_payload(
    *,
    ...
    payload: Payload = {
        "schema_version": 1,
        "ledger_id": "phase117.acceptance-fixture.v1",
...
def _write_ledger(
    ...
    *,
    ...
    for candidate in scan_historical_claim_sources(
        ...
        claim_id = claims_by_path.get(candidate.repository_path)
        review["claim_ids"] = [] if claim_id is None else [claim_id]
        review["exclusion"] = (
            {
    ...
    reviews.sort(
        key=lambda review: (review["source_kind"], review["repository_path"]),
    )
...
def acceptance_repository(
    ...
    return _build_acceptance_repository(
        tmp_path_factory.mktemp("phase117-acceptance-repository"),
    )
...
def test_final_held_out_run_source_drift_is_a_durable_error(
    ...
    drift_path = repository / "drift-during-final-run.txt"

    def run_then_drift(self: RuntimeSession):
        result = original_run(self)
        drift_path.write_text("post-execution source drift\n", encoding="utf-8")
        return result
