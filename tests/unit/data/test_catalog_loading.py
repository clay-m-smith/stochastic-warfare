...
class TestPhase28dSignatures:
    ...
    def test_all_new_units_have_signatures(self, unit_loader: UnitLoader, sig_loader: SignatureLoader) -> None:
...
class TestPhase28CrossRef:
    ...
    def test_weapon_ammo_refs_resolve(self, weapon_loader: WeaponLoader, ammo_loader: AmmoLoader) -> None:
        ...
        for wid in _NEW_WEAPON_IDS:
            ...
            for aid in defn.compatible_ammo:
                assert aid in ammo_loader.available_ammo(), f"Weapon {wid} references unknown ammo {aid}"
    ...
    def test_org_subordinate_unit_types_exist(self, unit_loader: UnitLoader) -> None:
        ...
        for rel_path in _ORG_FILES:
            ...
            for sub in toe.subordinates:
                assert sub.unit_type in available, f"Org {toe.name}: unknown unit_type {sub.unit_type}"
