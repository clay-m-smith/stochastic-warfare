...
HISTORICAL_ERA = "ancient_medieval"
...
class TestAncientMedievalUnits:
    ...
    def test_unit_domain_ground(self, era_unit_loader, unit_type: str) -> None:
        defn = era_unit_loader._definitions[unit_type]
        assert defn.domain == "ground"

    def test_cavalry_speed(self, era_unit_loader) -> None:
        defn = era_unit_loader._definitions["mongol_horse_archer"]
        assert defn.max_speed >= 7.0

    def test_infantry_speed(self, era_unit_loader) -> None:
        defn = era_unit_loader._definitions["roman_legionary_cohort"]
        assert 1.0 <= defn.max_speed <= 2.0

    def test_knight_armor(self, era_unit_loader) -> None:
        defn = era_unit_loader._definitions["norman_knight_conroi"]
        assert defn.armor_front >= 3.0

    def test_phalanx_slow(self, era_unit_loader) -> None:
        defn = era_unit_loader._definitions["greek_hoplite_phalanx"]
...
class TestAncientMedievalWeapons:
    ...
    def test_weapon_has_weapon_id(self, era_weapon_loader, weapon_id: str) -> None:
        defn = era_weapon_loader._definitions[weapon_id]
        assert defn.weapon_id == weapon_id

    def test_longbow_category(self, era_weapon_loader) -> None:
        defn = era_weapon_loader._definitions["longbow"]
        assert defn.category == "RIFLE"

    def test_gladius_category(self, era_weapon_loader) -> None:
        defn = era_weapon_loader._definitions["gladius"]
        assert defn.category == "MELEE"

    def test_catapult_category(self, era_weapon_loader) -> None:
        defn = era_weapon_loader._definitions["catapult"]
...
class TestAncientMedievalAmmo:
    ...
    def test_ammo_has_display_name(self, era_ammo_loader, ammo_id: str) -> None:
        defn = era_ammo_loader._definitions[ammo_id]
        assert defn.display_name

    def test_arrow_no_blast(self, era_ammo_loader) -> None:
        defn = era_ammo_loader._definitions["arrow_longbow"]
        assert defn.blast_radius_m == 0.0

    def test_stone_catapult_has_mass(self, era_ammo_loader) -> None:
        defn = era_ammo_loader._definitions["stone_catapult"]
        assert defn.mass_kg > 0

    def test_sling_stone_exists(self, era_ammo_loader) -> None:
        defn = era_ammo_loader._definitions["sling_stone"]
...
class TestAncientMedievalSensors:
    ...
    def test_watchtower_long_range(self, era_sensor_loader) -> None:
        defn = era_sensor_loader._definitions["watchtower"]
        assert defn.max_range_m >= 8000.0

    def test_watchtower_full_fov(self, era_sensor_loader) -> None:
        defn = era_sensor_loader._definitions["watchtower"]
        assert defn.fov_deg == 360.0

    def test_scout_wide_fov(self, era_sensor_loader) -> None:
        defn = era_sensor_loader._definitions["mounted_scout_ancient"]
        assert defn.fov_deg >= 90.0

    def test_ship_lookout_range(self, era_sensor_loader) -> None:
        defn = era_sensor_loader._definitions["ship_lookout"]
...
class TestAncientMedievalSignatures:
    ...
    def test_zeroed_thermal(self, era_sig_loader, profile_id: str) -> None:
        prof = era_sig_loader._profiles[profile_id]
        assert prof.thermal.emissivity == 0.0

    @pytest.mark.parametrize("profile_id", _AM_SIGS)
    def test_zeroed_radar(self, era_sig_loader, profile_id: str) -> None:
        prof = era_sig_loader._profiles[profile_id]
        assert prof.radar.rcs_frontal_m2 == 0.0

    def test_cavalry_taller_than_infantry(self, era_sig_loader) -> None:
        inf = era_sig_loader._profiles["roman_legionary_cohort"]
        cav = era_sig_loader._profiles["norman_knight_conroi"]
        assert cav.visual.height_m > inf.visual.height_m

    def test_cavalry_noisier_than_infantry(self, era_sig_loader) -> None:
        inf = era_sig_loader._profiles["english_longbowman"]
        cav = era_sig_loader._profiles["norman_knight_conroi"]
...
class TestMeleeWeaponProperties:
    ...
    def test_melee_max_range_short(self, era_weapon_loader, weapon_id: str) -> None:
        defn = era_weapon_loader._definitions[weapon_id]
        assert defn.max_range_m <= 5.0

    @pytest.mark.parametrize("weapon_id", _MELEE_WEAPONS)
    def test_melee_zero_muzzle_velocity(self, era_weapon_loader, weapon_id: str) -> None:
        defn = era_weapon_loader._definitions[weapon_id]
        assert defn.muzzle_velocity_mps == 0.0

    @pytest.mark.parametrize("weapon_id", _RANGED_WEAPONS)
    def test_ranged_has_compatible_ammo(self, era_weapon_loader, weapon_id: str) -> None:
        defn = era_weapon_loader._definitions[weapon_id]
        assert len(defn.compatible_ammo) > 0

    @pytest.mark.parametrize("weapon_id", _SIEGE_WEAPONS)
    def test_siege_requires_deployed(self, era_weapon_loader, weapon_id: str) -> None:
        defn = era_weapon_loader._definitions[weapon_id]
        assert defn.requires_deployed is True

    def test_battering_ram_requires_deployed(self, era_weapon_loader) -> None:
        defn = era_weapon_loader._definitions["battering_ram"]
        assert defn.requires_deployed is True

    def test_longbow_has_arrow_ammo(self, era_weapon_loader) -> None:
        defn = era_weapon_loader._definitions["longbow"]
...
class TestBackwardCompatibility:
    ...
    def test_ww1_has_sparse_physics_declaration(self) -> None:
        from stochastic_warfare.core.era import get_era_config
    ...
    def test_napoleonic_has_sparse_physics_declaration(self) -> None:
        from stochastic_warfare.core.era import get_era_config
