...
class TestSA6Gainful:
    ...
    def test_weapon_ammo_cross_ref(self, weapon_loader: WeaponLoader, ammo_loader: AmmoLoader) -> None:
        defn = weapon_loader.get_definition("sa6_3m9")
        ...
        for side in data["sides"]:
            for unit in side["units"]:
                if scenario_name in (
                    ...
                    assert unit["unit_type"] != "us_rifle_squad", f"{scenario_name} still uses us_rifle_squad"
