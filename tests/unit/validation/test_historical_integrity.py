...
def _write_claim_ledger(
    ...
    *,
    ...
    reviewed_identities = {(review["source_kind"], canonical_sha256(review["matches"])) for review in reviews}
    ...
    for candidate in contract.scan_historical_claim_sources(
        ...
        key = (
            ...
            canonical_sha256(candidate.model_dump(mode="json")["matches"]),
        )
    ...
    for identity, candidates in candidate_groups.items():
        claim_ids = sorted(
            ...
            for claim_id in scenario_claim_ids_by_path.get(
                candidate.repository_path,
                [],
            )
        )
        if claim_ids and identity not in reviewed_identities:
            reviews.append(
                _review_candidate(
                    candidates[0],
                    claim_ids=claim_ids,
    ...
    payload: dict[str, Any] = {
        "schema_version": 2,
        "ledger_id": "phase117.path-integrity.v2",
...
def _review_candidate(
    ...
    *,
    ...
    payload["source_occurrences"] = source_occurrences
...
def test_strict_claim_ledger_audits_the_exact_repository_inventory() -> None:
    ...
    candidates = contract.scan_historical_claim_sources(ROOT)
    candidate_counts = {
        kind: sum(candidate.source_kind is kind for candidate in candidates) for kind in contract.ClaimSourceKind
    }
    scenario_claims = tuple(
        claim for claim in ledger.claims if claim.surface is contract.ClaimSurface.SCENARIO_DOCUMENTED_OUTCOMES
    )
    documentation_claims = tuple(
        claim for claim in ledger.claims if claim.surface is contract.ClaimSurface.DOCUMENTATION_CLAIM
    )

    assert ledger.claims
    assert audit.scenario_collections == len(scenario_claims)
    assert audit.scenario_metrics == sum(len(claim.metric_scope) for claim in scenario_claims)
    assert audit.python_test_surfaces == sum(
        claim.surface
        in {
            contract.ClaimSurface.CURRENT_ENGINE_REGRESSION_SNAPSHOT,
            contract.ClaimSurface.PYTHON_HISTORICAL_CLAIM_TEST,
        }
        for claim in ledger.claims
    )
    assert audit.frontend_test_surfaces == 0
    assert audit.documentation_claims == len(documentation_claims)
    assert audit.documentation_claim_paths == len(
        {claim.repository_path for claim in documentation_claims},
    )
    assert audit.api_python_candidate_paths == candidate_counts[contract.ClaimSourceKind.API_PYTHON]
    assert audit.frontend_public_candidate_paths == candidate_counts[contract.ClaimSourceKind.FRONTEND_PUBLIC_SOURCE]
    assert audit.frontend_test_candidate_paths == 0
    assert audit.python_test_candidate_paths == candidate_counts[contract.ClaimSourceKind.PYTHON_TEST]
    assert audit.public_document_candidate_paths == candidate_counts[contract.ClaimSourceKind.PUBLIC_DOCUMENT]
    assert audit.scenario_yaml_candidate_paths == candidate_counts[contract.ClaimSourceKind.SCENARIO_YAML]
    assert audit.workflow_document_candidate_paths == candidate_counts[contract.ClaimSourceKind.WORKFLOW_DOCUMENT]
    assert audit.claim_bound_source_reviews == sum(bool(review.claim_ids) for review in ledger.claim_source_reviews)
    assert audit.reviewed_nonclaim_source_reviews == sum(
        review.exclusion is not None for review in ledger.claim_source_reviews
    )
    assert audit.claim_bound_source_reviews + audit.reviewed_nonclaim_source_reviews == len(ledger.claim_source_reviews)
...
def test_scenario_catalog_loader_audits_published_claims_without_packaging_docs(
    ...
    documentation_claim = _unsupported_claim(
        ...
        repository_path="docs/guide/missing.md",
        scenario_path=None,
        surface="documentation_claim",
        locator={
            "kind": "required_text",
            "text": "Synthetic historical validation disclosure.",
    ...
    document_path = tmp_path / "docs/guide/missing.md"
    document_path.parent.mkdir(parents=True)
    document_path.write_text(
        "Synthetic historical validation disclosure.\n",
        encoding="utf-8",
    )
    contract = _contract_module()
    documentation_candidate = next(
        candidate
        for candidate in contract.scan_historical_claim_sources(tmp_path)
        if candidate.repository_path == "docs/guide/missing.md"
    )
    ...
    packaged_ledger = loader.load_packaged_scenario_catalog(ledger_path)
    ...
    assert tuple(claim.claim_id for claim in packaged_ledger.claims) == ("scenario.synthetic.catalog",)
    assert packaged_ledger.schema_version == 2
    assert packaged_ledger.claim_source_scanner_version == 3
...
def test_closed_vocabulary_scanner_finds_python_and_public_doc_claims(
    ...
    python_path = tmp_path / "tests/validation/historical/test_accuracy.py"
    ...
    document_path = tmp_path / "docs/guide/report.md"
    document_path.parent.mkdir(parents=True)
    document_path.write_text(
...
def test_scanner_v3_discovers_only_current_truth_source_kinds(
    tmp_path: Path,
) -> None:
    sources = {
        ...
        "frontend/src/types/openapi.generated.ts": (
            'export type HistoricalClaimDisposition = "production_validated"\n'
        ),
        "tests/validation/historical/test_accuracy.py": ("CURRENT_ENGINE_TERMINAL_SNAPSHOT = {}\n"),
        "data/scenarios/prose/scenario.yaml": ("name: prose\n# This historical outcome is validated.\n"),
        ".github/workflows/historical.yml": ("name: Historical validation evidence\njobs: {}\n"),
    }
    ...
    assert tuple((candidate.source_kind.value, candidate.repository_path) for candidate in candidates) == (
        ...
        (
            "workflow_document",
            ".github/workflows/historical.yml",
        ),
    )
    candidates_by_path = {candidate.repository_path: candidate for candidate in candidates}
    assert "historical_status_vocabulary" in {
        match.rule_id.value for match in candidates_by_path["api/schemas.py"].matches
    }
    assert "regression_snapshot" in {
        match.rule_id.value for match in candidates_by_path["tests/validation/historical/test_accuracy.py"].matches
    }
...
def test_scanner_v3_normalizes_public_status_vocabulary(
    tmp_path: Path,
    status_source: str,
) -> None:
...
def test_frontend_declaration_and_test_sources_are_out_of_scope(
    tmp_path: Path,
) -> None:
    declaration = tmp_path / "frontend/src/historical-status.d.ts"
    ...
    frontend_test = tmp_path / "frontend/src/__tests__/Status.test.tsx"
    frontend_test.parent.mkdir(parents=True)
    frontend_test.write_text(
        "const status = 'historical outcome validated'\n",
        encoding="utf-8",
    )
    contract = _contract_module()

    assert contract.scan_historical_claim_sources(tmp_path) == ()
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            ...
            ".github/workflows/historical.yml",
            "documentation_claim",
            "name: Historical validation remains unsupported\njobs: {}\n",
            "Historical validation remains unsupported",
...
def test_scanner_v3_rejects_claim_appends_to_reviewed_non_scenario_sources(
    tmp_path: Path,
    source_kind: str,
    repository_path: str,
    surface: str,
    ...
    with pytest.raises(
        ValueError,
        match=rf"unreviewed_claim_source_paths=.*{re.escape(repository_path)}",
    ):
        loader.load(ledger_path)
    if packaged:
        with pytest.raises(
...
def test_full_and_packaged_loaders_reject_scenario_prose_append(
    ...
    for load in (loader.load, loader.load_scenario_catalog):
        with pytest.raises(
            ValueError,
            match=r"unreviewed_claim_source_paths=.*inventoried",
        ):
            load(ledger_path)
...
def test_scenario_review_must_bind_collection_and_prose_claims(
    ...
    with pytest.raises(ValueError, match="do not exactly bind"):
        contract.HistoricalClaimLedgerLoader(tmp_path).load(ledger_path)


def test_semantic_identity_is_lf_normalized_and_candidate_order_is_stable(
    tmp_path: Path,
) -> None:
...
def test_scenario_scanner_rejects_duplicate_yaml_keys(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="cannot scan YAML"):
...
def test_generic_tests_devlogs_and_mirrored_skills_are_out_of_scope(
    tmp_path: Path,
) -> None:
    sources = {
        "tests/validation/test_synthetic_claim.py": "Historical outcome validated.\n",
        "docs/devlog/phase-999.md": "Historical outcome validated.\n",
        "docs/brainstorm-block999.md": "Historical outcome validated.\n",
        "docs/development-phases-block999.md": "Historical outcome validated.\n",
        ".agents/skills/backtest/SKILL.md": "Historical outcome validated.\n",
        ".claude/skills/backtest/SKILL.md": "Historical outcome validated.\n",
    }
    for relative, source in sources.items():
        path = tmp_path / relative
        path.parent.mkdir(parents=True, exist_ok=True)
...
@pytest.mark.test_evidence("behavioral_oracle")
def test_schema_v1_scanner_v2_ledger_is_rejected_after_v3_migration(
    tmp_path: Path,
) -> None:
    claim = _write_synthetic_scenario_claim(tmp_path)
    ledger_path = _write_claim_ledger(tmp_path, [claim])
    ...
    payload["claim_source_scanner_version"] = 2
    ...
    with pytest.raises(ValueError, match="schema_version.*strict integer 2"):
        contract.HistoricalClaimLedgerLoader(tmp_path).load(ledger_path)
...
@pytest.mark.parametrize(
    ...
    (
        ("semantic_span_sha256", "0" * 64, "sentinel digest"),
        (
            "exclusion",
            {
...
def test_reviewed_source_schema_rejects_sentinel_or_paper_exclusions(
    ...
    source_path = tmp_path / "docs/guide/history.md"
    ...
    if field == "semantic_span_sha256":
        review["matches"][0][field] = replacement
    else:
...
def test_semantic_review_ignores_unrelated_edits_and_path_moves_but_rejects_matched_edits(
    tmp_path: Path,
) -> None:
    claim = _write_synthetic_scenario_claim(tmp_path)
    source_path = tmp_path / "docs/guide/synthetic-claim.md"
    source_path.parent.mkdir(parents=True)
    ...
    with pytest.raises(
        ValueError,
        match=r"unreviewed_claim_source_paths=.*synthetic-claim.md",
    ):
        loader.load(ledger_path)
    ...
    mismatched_review["source_occurrences"] += 1
    ledger_path = _write_claim_ledger(
        tmp_path,
        [claim],
    ...
    with pytest.raises(
        ValueError,
        match=r"claim_source_rule_mismatches=",
    ):
        loader.load(ledger_path)
    ...
    assert audit.public_document_candidate_paths == 1
    assert audit.reviewed_nonclaim_source_reviews == 1
    assert audit.unreviewed_claim_source_paths == ()

    source_path.write_text(
        source.rstrip() + " Ordinary implementation note with no claim vocabulary.\n",
        encoding="utf-8",
    )
    loader.load(ledger_path)

    moved_path = tmp_path / "docs/reference/synthetic-claim.md"
    moved_path.parent.mkdir(parents=True)
    source_path.rename(moved_path)
    loader.load(ledger_path)

    moved_path.write_text(
        "Current winner asserted against a different historical result.\n",
        encoding="utf-8",
    )
    with pytest.raises(
        ValueError,
...
def test_semantic_review_rejects_a_changed_numeric_claim_value(
    tmp_path: Path,
) -> None:
    claim = _write_synthetic_scenario_claim(tmp_path)
    source_path = tmp_path / "docs/guide/numeric-claim.md"
    source_path.parent.mkdir(parents=True)
    source_path.write_text(
        "Historical accuracy is 0.95. Ordinary implementation note.\n",
        encoding="utf-8",
    )
    contract = _contract_module()
    candidate = contract.scan_historical_claim_sources(tmp_path)[0]
    ...
    source_path.write_text(
        "Historical accuracy is 0.90. Ordinary implementation note.\n",
        encoding="utf-8",
    )

    with pytest.raises(
        ValueError,
        match=r"unreviewed_claim_source_paths=.*numeric-claim.md",
    ):
        loader.load(ledger_path)
...
def test_full_loader_rejects_stale_claim_source_review(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        ValueError,
        match=r"stale_claim_source_reviews=",
    ):
        contract.HistoricalClaimLedgerLoader(tmp_path).load(ledger_path)


def test_explicit_python_historical_claim_contract_is_exactly_review_bound(
    tmp_path: Path,
) -> None:
    source = '"""This regression is unsupported as historical validation."""\n'
    source_path = tmp_path / "tests/validation/historical/test_accuracy.py"
    source_path.parent.mkdir(parents=True)
    source_path.write_text(source, encoding="utf-8")
    claim = _unsupported_claim(
        claim_id="test.validation.current-regression.historical-claim",
        repository_path="tests/validation/historical/test_accuracy.py",
        scenario_path=None,
