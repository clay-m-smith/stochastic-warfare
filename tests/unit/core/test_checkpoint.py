...
from stochastic_warfare.legacy import checkpoint_pickle
from stochastic_warfare.legacy.checkpoint_pickle import (
...
class TestLegacyPickleMigration:
    """Keep unsafe pickle decoding outside the runtime checkpoint path."""

    def test_runtime_restore_rejects_legacy_pickle(self) -> None:
        legacy_payload = {
            "version": 1,
        ...
        with pytest.raises(
            ValueError,
            match="not accepted by runtime restore",
        ):
            mgr.restore_checkpoint(legacy_data)

    def test_migration_requires_explicit_trust(
        self,
        ...
        monkeypatch.setattr(checkpoint_pickle.pickle, "loads", fake_loads)
        with pytest.raises(
            UnsafeLegacyCheckpointError,
            match="explicit trusted=True",
        ):
            convert_trusted_pickle_checkpoint(b"\x80payload")
        assert called is False

    def test_trusted_migration_with_numpy_produces_runtime_json(self) -> None:
        legacy_payload = {
            "version": 1,
            "clock": {"tick_count": 0},
            "rng": {"master_seed": 1},
        ...
        restored = CheckpointManager().restore_checkpoint(converted)
    ...
    def test_explicit_migration_cli_writes_json(self, tmp_path) -> None:
        legacy_payload = {
            "version": 1,
            "clock": {"tick_count": 5},
            "rng": {"master_seed": 42},
        ...
        assert (
            checkpoint_pickle.main(
                [str(path), str(output), "--trusted-input"],
            )
            == 0
        )
        restored = CheckpointManager().restore_checkpoint(output.read_bytes())
