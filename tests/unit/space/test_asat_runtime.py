...
def test_observer_failure_cannot_rollback_or_retry_committed_order(
    ...
    observed: list[Event] = []
    failure = RuntimeError("phase110 observer failure")
    ...
    with caplog.at_level(
        ...
        with pytest.raises(RuntimeError) as caught:
            asat.execute_due_orders(0.0, TS)
    ...
    assert state["completed_orders"]["phase110_order_0"]["outcome"] == "hit"


def test_degraded_observer_failures_are_reported_in_notification_order(
) -> None:
    _, asat, manager, bus, _ = _runtime(target_count=1)
    first = RuntimeError("first observer failure")
    second = RuntimeError("second observer failure")
    ...
    class DegradedPolicy:
        def suppress(
            ...
            failures.append((subsystem, operation, failure))
            return True

    policy = DegradedPolicy()
    asat.bind_runtime_failure_handler(policy.suppress)

    def fail_first(_event: ConstellationDegradedEvent) -> None:
        raise first

    def fail_second(_event: ConstellationDegradedEvent) -> None:
        raise second

    bus.subscribe(ConstellationDegradedEvent, fail_first)
    bus.subscribe(ConstellationDegradedEvent, fail_second)

    result = asat.execute_due_orders(0.0, TS)

    assert result[0]["outcome"] == "hit"
    assert manager.active_count(CONSTELLATION_ID) == 0
    assert failures == [
        ("space.asat", "publish_committed_event", first),
        ("space.asat", "publish_committed_event", second),
    ]
