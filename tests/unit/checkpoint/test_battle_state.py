...
def _add_deferred_order(
    bm: Any,
    unit_id: str,
    result: PropagationResult | None = None,
) -> None:
    battle_id = "battle_0000"
    battle = bm._battles.get(battle_id)
    if battle is None:
        ...
        bm._battles[battle_id] = battle
        bm._next_battle_id = 1
    battle.unit_ids.add(unit_id)
    bm._pending_decisions[unit_id] = 30.0
    bm._misinterpreted_orders[unit_id] = result or _misinterpreted_order()
    bm._deferred_battle_ids[unit_id] = battle_id
...
class TestSetStateRestore:
    ...
    def test_restore_misinterpreted_orders(self):
        source = _make_battle_manager()
        _add_deferred_order(source, "u1")
