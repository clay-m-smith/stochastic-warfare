...
class TestResolveAirEngagement:
    ...
    def test_effective_bvr_range_changes_only_bvr_distance(self) -> None:
        e = _engine()
        result = e.resolve_air_engagement(
            "a1",
            "t1",
            Position(0, 0, 10_000),
            Position(0, 85_000, 10_000),
            effective_bvr_range_modifier=1.1,
        )

        assert result.mode == AirCombatMode.BVR
        assert result.range_m == pytest.approx(85_000.0)

    @pytest.mark.parametrize("modifier", [0.0, -1.0, float("inf"), float("nan"), True])
    def test_effective_bvr_range_rejects_non_positive_or_non_finite_values(
        self,
        modifier: float,
    ) -> None:
        with pytest.raises(ValueError, match="finite and positive"):
            _engine().resolve_air_engagement(
                "a1",
                "t1",
                Position(0, 0, 10_000),
                Position(0, 50_000, 10_000),
                effective_bvr_range_modifier=modifier,
            )
...
class TestDeterminism:
    def test_default_bvr_modifier_is_bit_exact_with_explicit_one(self) -> None:
        omitted = _engine(42)
        explicit = _engine(42)
        args = (
            "a1",
        ...
        omitted_result = omitted.resolve_air_engagement(*args)
        explicit_result = explicit.resolve_air_engagement(
            *args,
            effective_bvr_range_modifier=1.0,
        )

        assert omitted_result == explicit_result
        assert omitted.get_state() == explicit.get_state()
