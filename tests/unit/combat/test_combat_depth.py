...
class TestProductionEngagementInputs:
    """Training, detection, concealment, and default targeting use the executor."""
    ...
    @pytest.mark.parametrize(
        ("training_level", "expected_skill"),
        [(0.0, 0.4), (0.5, 0.6), (1.0, 0.8)],
    )
    def test_training_boundaries_reach_route_engagement(
        self,
        training_level: float,
        expected_skill: float,
    ) -> None:
        attacker = make_executor_unit(
            "attacker",
            "blue",
            0.0,
            training_level=training_level,
        )
        target = make_executor_unit("target", "red", 1_000.0)
        ...
        context = make_context(
            ...
            calibration={"visibility_m": 2_000.0},
        ...
        assert recorder.calls[0]["crew_skill"] == pytest.approx(expected_skill)

    @pytest.mark.parametrize(
        ("snr_db", "expected_visibility"),
        [(None, 1.0), (-20.0, 0.3), (10.0, 10.0 ** 0.5 / 10.0), (40.0, 1.0)],
    )
    def test_detection_quality_boundaries_reach_route_engagement(
        self,
        snr_db: float | None,
        ...
        detection = (
            ...
            if snr_db is None
                ...
                compute_snr_visual=lambda *_args, **_kwargs: snr_db,
            )
        )
        context = make_context(
            {"blue": [attacker], "red": [target]},
            unit_weapons={attacker.entity_id: [(weapon, ammunition)]},
            unit_sensors={attacker.entity_id: [VisualSensor(2_000.0)]},
            calibration={"visibility_m": 2_000.0},
            engagement_engine=recorder,
            detection_engine=detection,
        )

        execute_engagement_side(self._bm(), context, "blue", [target])

        assert recorder.calls[0]["visibility"] == pytest.approx(
            expected_visibility,
        )

    @pytest.mark.parametrize(
        ("concealment", "expected_calls"),
        [(0.0, 1), (0.5, 0)],
    )
    def test_concealment_changes_real_detection_reach(
        self,
        concealment: float,
        expected_calls: int,
    ) -> None:
    ...
    def test_default_target_mode_routes_the_higher_threat(self) -> None:
        attacker = make_executor_unit("attacker", "blue", 0.0)
        close_infantry = make_executor_unit("infantry", "red", 1_000.0)
        high_value_hq = make_executor_unit("hq", "red", 1_500.0)
        object.__setattr__(
            high_value_hq,
            "support_type",
        ...
        recorder = RecordingEngagementEngine()
        context = make_context(
            {"blue": [attacker], "red": [close_infantry, high_value_hq]},
            unit_weapons={attacker.entity_id: [(weapon, ammunition)]},
            calibration={"visibility_m": 2_000.0},
            engagement_engine=recorder,
        )
        ...
        execute_engagement_side(
            ...
            context,
            "blue",
            [close_infantry, high_value_hq],
        )

        assert [call["target_id"] for call in recorder.calls] == ["hq"]
