...
class TestNavalPosture:
    ...
    @pytest.mark.parametrize(
        ("posture_name", "expected_calls"),
        [("ANCHORED", 0), ("UNDERWAY", 1)],
    )
    def test_posture_gates_real_engagement_route(
        self,
        posture_name: str,
        expected_calls: int,
    ) -> None:
        ...
        weapon, ammunition = make_executor_weapon(
            ...
            target_domains=[Domain.NAVAL.name],
        )
...
class TestDEWDisablePath:
    ...
    def test_beam_wavelength_used_in_laser_engagement(self):
        ...
        engine.execute_laser_engagement(
