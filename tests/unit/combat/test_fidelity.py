...
class TestPerSideTargetSizeModifier:
    """The production executor resolves target-side overrides and fallbacks."""

    @pytest.mark.parametrize(
        ("target_side", "calibration", "expected_modifier"),
        [
            ("red", {"target_size_modifier_red": 0.5}, 0.5),
            ("blue", {"target_size_modifier_red": 0.5}, 1.0),
            ("red", {"target_size_modifier": 0.7}, 0.7),
            (
                "blue",
                {
                    "target_size_modifier_red": 0.5,
    ...
    def test_route_engagement_receives_target_side_modifier(
        self,
        target_side: str,
        calibration: dict[str, float],
        expected_modifier: float,
    ) -> None:
        attacker_side = "red" if target_side == "blue" else "blue"
        attacker = make_executor_unit("attacker", attacker_side, 0.0)
        target = make_executor_unit("target", target_side, 1_000.0)
        weapon, ammunition = make_executor_weapon()
        recorder = RecordingEngagementEngine()
        context = make_context(
            {attacker_side: [attacker], target_side: [target]},
            unit_weapons={attacker.entity_id: [(weapon, ammunition)]},
            calibration={"visibility_m": 2_000.0, **calibration},
        ...
        assert recorder.calls[0]["target_size_m2"] == pytest.approx(
            8.5 * expected_modifier,
        )
