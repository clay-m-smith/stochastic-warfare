...
class TestHoldFire:
    """Hold-fire gates the real engagement route at its closed boundary."""

    @pytest.mark.parametrize(
        ("enabled", "range_m", "expected_calls"),
        [
            (True, 2_500.0, 0),
            (True, 2_400.0, 1),
            (False, 2_500.0, 1),
    ...
    def test_executor_hold_fire_gate(
        self,
        enabled: bool,
        range_m: float,
        expected_calls: int,
        ...
        target = make_executor_unit("target", "red", range_m)
        weapon, ammunition = make_executor_weapon(max_range_m=3_000.0)
        recorder = RecordingEngagementEngine()
        context = make_context(
            {"blue": [attacker], "red": [target]},
            unit_weapons={attacker.entity_id: [(weapon, ammunition)]},
            calibration={"visibility_m": 3_000.0},
            behavior_rules=(
                {"blue": {"hold_fire_until_effective_range": True}}
                if enabled
                else {}
            ),
