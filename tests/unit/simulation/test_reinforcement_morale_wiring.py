...
def test_failed_wave_is_atomic_and_retryable(
    ...
    retry_ctx = _load(REINFORCEMENT_SCENARIO, config)
    retry_engine = SimulationEngine(
        retry_ctx,
        config=EngineConfig(resolution_closing_range_mult=0.0),
        campaign_config=_quiet_campaign_config(),
    )
    retry_engine.restore(healthy_checkpoint)
    retry_events: list[ReinforcementArrivedEvent] = []
    retry_ctx.event_bus.subscribe(
        ReinforcementArrivedEvent,
        retry_events.append,
    )
    retry_engine.step()

    control_ctx = _load(REINFORCEMENT_SCENARIO, config)
    control_engine = SimulationEngine(
        control_ctx,
        config=EngineConfig(resolution_closing_range_mult=0.0),
        campaign_config=_quiet_campaign_config(),
    )
    ...
    retry_entry = retry_engine.campaign_manager._reinforcements[0]
    assert retry_entry.arrived is True
    assert len(retry_ctx.units_by_side["blue"]) == len(before_units["blue"]) + 2
...
def test_versionless_current_morale_envelope_rejects_atomically() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="Versionless battle state cannot contain a deferred-OODA schema marker",
    ):
