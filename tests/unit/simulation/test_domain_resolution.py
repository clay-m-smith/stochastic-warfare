...
class TestVolleyFireRouting:
    ...
    def test_formation_firepower_fraction_read(self):
        ...
        form_eng = MagicMock()
        form_eng.firepower_fraction.return_value = 0.5
        ...
        form_eng.firepower_fraction.assert_called_once_with(attacker.entity_id)
        call_kw = vf.fire_volley.call_args[1]
...
class TestGetFormationFirepower:
    ...
    def test_engine_queried(self):
        eng = MagicMock()
        eng.firepower_fraction.return_value = 0.6
        ctx = SimpleNamespace(formation_napoleonic_engine=eng)
        u = _make_unit("u1")
        result = _get_formation_firepower(ctx, u)
        assert result == 0.6
        eng.firepower_fraction.assert_called_once_with("u1")
