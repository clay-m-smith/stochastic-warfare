...
class TestStrictMode:
    """Strict execution propagates; explicit degraded execution records."""
    ...
    def test_non_strict_weather_error_does_not_raise_structural_diagnostic(
        ...
        assert any("environment.weather.update" in record.getMessage() for record in caplog.records)
    ...
    def test_non_strict_space_error_does_not_raise_structural_diagnostic(
        ...
        assert any("space.update" in record.getMessage() for record in caplog.records)
...
class TestEWUpdate:
    ...
    def test_ew_error_emits_structural_diagnostic_without_raising(
        ...
        engine._update_ew(10.0)
        mock_ew.update.assert_called_once_with(10.0)
        assert any("ew.jamming.update" in record.getMessage() for record in caplog.records)
...
class TestMOPPSpeed:
    ...
    def test_mopp_slows_movement(self) -> None:
        ...
        mock_cbrn = MagicMock()
        mock_cbrn.get_mopp_level.side_effect = lambda unit_id: (
            4 if unit_id == "u1" else 0
        )
    ...
    def test_mopp_0_no_penalty_in_movement(self) -> None:
        ...
        mock_cbrn = MagicMock()
        mock_cbrn.get_mopp_level.return_value = 0
    ...
    def test_mopp_missing_unit_no_penalty(self) -> None:
        """The public CBRN query returns MOPP 0 for an unregistered unit."""
        bm = BattleManager(EventBus())
    ...
    def test_cbrn_without_public_mopp_query_is_rejected(self) -> None:
        """A present CBRN owner must implement its public level query."""
        bm = BattleManager(EventBus())
        ...
        active_enemies = {"blue": [e1], "red": [u1]}
        with pytest.raises(AttributeError, match="get_mopp_level"):
            bm._execute_movement(
                ctx,
                ctx.units_by_side,
                active_enemies,
                10.0,
...
class TestErrorHandling:
    ...
    def test_cbrn_error_emits_structural_diagnostic_without_raising(
        ...
        assert any("combat.cbrn.update" in record.getMessage() for record in caplog.records)
    ...
    def test_multiple_environment_errors_do_not_raise_structural_diagnostic(
        ...
        assert any("environment.weather.update" in message for message in messages)
        assert any("environment.sea_state.update" in message for message in messages)
