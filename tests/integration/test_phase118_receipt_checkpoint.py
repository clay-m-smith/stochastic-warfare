...
@pytest.mark.parametrize("flag", UNSUPPORTED_MODEL_CONTROLS)
def test_runtime_rejects_retired_flat_calibration_drift_before_any_step_mutation(
    flag: str,
) -> None:
    session = _build_session()
    before = session.engine.checkpoint()
    ...
    assert flag in message
    assert "unsupported" in message.lower()
    session.context.cal_flat[flag] = False
    assert session.engine.checkpoint() == before


@pytest.mark.parametrize(("field_name", "invalid_value"), UNSUPPORTED_LOD_TUNING)
def test_runtime_rejects_retired_lod_tuning_drift_before_any_step_mutation(
    field_name: str,
    invalid_value: int,
) -> None:
    session = _build_session()
    ...
    assert "unsupported" in message.lower()
    session.context.cal_flat[field_name] = original_value
    assert session.engine.checkpoint() == before
...
def test_receipt_exposure_rejects_unsupported_flat_calibration_drift(
    field_name: str,
    invalid_value: bool | int,
) -> None:
    session = _build_session()
...
def test_receipt_exposure_rejects_supported_flat_calibration_drift() -> None:
    session = _build_session()
    before = session.performance_execution_receipt()
    original_value = session.context.cal_flat["enable_soa"]
    session.context.cal_flat["enable_soa"] = True

    with pytest.raises(RuntimeError, match="Flat performance calibration"):
        session.performance_execution_receipt()

    session.context.cal_flat["enable_soa"] = original_value
    assert session.performance_execution_receipt() == before
