...
def test_ready_delivery_report_n_failure_preserves_prior_commits_and_retries(
    ...
    with pytest.raises(SpaceISRDeliveryError) as retry_error:
        engine.step()
    assert retry_error.value is caught.value
    with pytest.raises(SpaceISRDeliveryError) as checkpoint_error:
        engine.checkpoint()
    assert checkpoint_error.value is caught.value
    assert fusion.delivery_receipts == prior_receipts
    assert tuple(report.report_id for report in isr.get_recent_reports()) == tuple(
        range(3, 49),
    )

    resumed = _engine(seed=333_112, config=_short_delay_config())
    resumed.restore(healthy_checkpoint)
    _advance_to(resumed, 14_700.0)
    resumed_fusion = resumed._ctx.fog_of_war.intel_fusion
    resumed_isr = resumed._ctx.space_engine.isr_engine
    assert tuple(
        receipt.report_id
        for receipt in resumed_fusion.delivery_receipts
    ) == tuple(range(1, 9))
    assert tuple(
        report.report_id
        for report in resumed_isr.get_recent_reports()
    ) == tuple(range(9, 49))

    control = _engine(config=_short_delay_config())
    _advance_to(control, 14_700.0)
