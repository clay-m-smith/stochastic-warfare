...
SIG_DIR = Path(__file__).resolve().parents[3] / "data" / "signatures"
SENSOR_DIR = Path(__file__).resolve().parents[3] / "data" / "sensors"
...
class TestThermalContrast:
    def test_high_contrast_better(self) -> None:
        ...
        thermal_output = profile.thermal.heat_output_kw * profile.thermal.emissivity
        snr_hi = DetectionEngine.compute_snr_thermal(sensor, thermal_output, 2000.0, 1.0)
        snr_lo = DetectionEngine.compute_snr_thermal(sensor, thermal_output, 2000.0, 0.1)
