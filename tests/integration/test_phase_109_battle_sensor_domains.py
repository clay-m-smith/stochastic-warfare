...
def _run_f16_engagement(
    ...
    ctx.calibration = ctx.calibration.model_copy(
        update={"target_selection_mode": "nearest"},
    )
    ctx.cal_flat = {
        **ctx.cal_flat,
...
def test_cooldown_blocked_sam_does_not_consume_engager_limit() -> None:
    ...
    ctx = ScenarioLoader(DATA_DIR).load(BEKAA_SCENARIO_PATH, seed=SEED)
    ctx.calibration = ctx.calibration.model_copy(
        update={
...
def _run_fallujah_night_engagement(
    ...
    ctx = ScenarioLoader(DATA_DIR).load(FALLUJAH_SCENARIO_PATH, seed=SEED)
    ctx.calibration = ctx.calibration.model_copy(
        update={"target_selection_mode": "nearest"},
    )
    ctx.cal_flat = {
