...
SKILL_SYNC = REPO_ROOT / "scripts" / "sync_repository_skills.py"
EXPECTED_ROUTES = (
    "audit-determinism",
    "backtest",
    "calibrate",
...
def test_claude_routes_exactly_mirror_canonical_repository_skills() -> None:
    ...
    for route in EXPECTED_ROUTES:
        ...
        if provider.is_symlink() or not provider.is_file():
            unsafe_projections.append(route)
    ...
    assert unsafe_projections == []
    assert mismatches == []
    assert obsolete_prompts == []


def test_repository_skill_projection_checker_reports_exact_routes() -> None:
    completed = subprocess.run(
        [sys.executable, str(SKILL_SYNC)],
        cwd=REPO_ROOT,
        check=False,
        capture_output=True,
        text=True,
    ...
    assert json.loads(completed.stdout) == {
        "canonical_routes": len(EXPECTED_ROUTES),
        "mismatched": [],
        "missing_or_stale": [],
        "unsafe": [],
    }
