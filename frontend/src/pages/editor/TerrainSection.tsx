...
export function TerrainSection({ config, dispatch }: TerrainSectionProps) {
  ...
  return (
    <section>
      <h3 className="mb-3 text-sm font-semibold uppercase tracking-wide text-gray-500">Terrain</h3>
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
        ...
        <label className="block">
          <span className="text-sm text-gray-700">Cell Size (m)</span>
          <input
            type="number"
            min={10}
            step={10}
        ...
        <label className="block">
          <span className="text-sm text-gray-700">Base Elevation (m)</span>
          <input
            type="number"
            step={10}
            className="mt-1 block w-full rounded border-gray-300 text-sm shadow-sm"
