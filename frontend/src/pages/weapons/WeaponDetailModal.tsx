...
import { useWeaponDetail } from '../../hooks/useMeta'
...
export function WeaponDetailModal({ weaponId, onClose }: WeaponDetailModalProps) {
  const { data, isLoading } = useWeaponDetail(weaponId ?? '')

  return (
    <Dialog open={!!weaponId} onClose={onClose} className="relative z-50">
      <div className="fixed inset-0 bg-black/30" aria-hidden="true" />
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <DialogPanel className="max-h-[80vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white p-6 shadow-xl dark:bg-gray-800">
          <DialogTitle className="mb-4 text-xl font-bold text-gray-900 dark:text-gray-100">
            {data?.definition?.name ? String(data.definition.name) : weaponId}
          </DialogTitle>
          ...
          <button
            onClick={onClose}
            aria-label="Close"
            className="mt-6 rounded-md bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600"
          >
            Close
          </button>
        </DialogPanel>
